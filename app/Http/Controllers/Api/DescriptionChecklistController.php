...
class DescriptionChecklistController extends Controller
{
    public function index(Request $request)
    {
        $query = DescriptionChecklist::query();
    ...
    public function store(Request $request)
    {
        ...
        if (!isset($data['order'])) {
            $maxOrder = DescriptionChecklist::max('order') ?? 0;
            $data['order'] = $maxOrder + 1;
        }

        $checklist = DescriptionChecklist::create($data);
    ...
    {
        $checklist = DescriptionChecklist::findOrFail($id);
