...
@section('title', 'سرویس‌های منقضی شده')
        ...
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
            <div class="widget widget-chart-one">
                <div class="widget-heading">
                    <h5 class="mb-0">سرویس‌های منقضی شده - <span id="org-name-expired">...</span></h5>
                </div>
                <div class="widget-content">
                    <div class="widget-content widget-content-area br-6">
                        @include('organization.components.datatable', [
                            'title' => 'لیست سرویس‌های منقضی شده',
                            'apiUrl' => '/api/organization/services/expired',
                            'createButton' => false,
...
window.onShowDetails = function(id) {
    ...
    $.ajax({
        url: `/api/organization/services/expired`,
        type: 'GET',
        headers: {
...
function displayServiceDetails(service) {
    ...
    // Assigned Information (if service was assigned before expiring)
    if (service.technician || service.assigned_at) {
        html += '<div class="card mb-3">';
        html += '<div class="card-header"><h6 class="mb-0">اطلاعات اختصاص</h6></div>';
        html += '<div class="card-body">';
        html += '<table class="table table-bordered">';
        if (service.technician) {
            html += '<tr><th width="30%">تکنسین:</th><td>' + (service.technician.first_name + ' ' + service.technician.last_name) + '</td></tr>';
            html += '<tr><th>شماره تماس تکنسین:</th><td>' + (service.technician.phone_number || '-') + '</td></tr>';
        }
        if (service.assigned_at_jalali) {
    ...
    // Completed Information with Checklist (if service was completed before expiring)
    if (service.status === 'expired' && service.checklist_data) {
        html += '<div class="card mb-3">';
        html += '<div class="card-header"><h6 class="mb-0">اطلاعات تکمیل</h6></div>';
        html += '<div class="card-body">';
        html += '<table class="table table-bordered mb-3">';
        if (service.completed_at_jalali) {
...
getOrganizationData(function(org, error) {
    if (!error && org) {
        $('#org-name-expired').text('آسانسور ' + org.name);
    }
});
</script>
@endsection
