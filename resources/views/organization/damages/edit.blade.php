...
@section('title', 'ویرایش گزارش خرابی')
            ...
            <div class="widget widget-chart-one">
                <div class="widget-heading">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">ویرایش گزارش خرابی</h5>
                        <div>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // Load damage data after buildings are loaded
                loadDamageData();
            }
        },
        error: function(xhr) {
...
function loadDamageData() {
    ...
    $.ajax({
        url: `/api/organization/damages/${damageId}`,
        type: 'GET',
        headers: {
            'Authorization': 'Bearer ' + token
        },
        success: function(response) {
            if (response.success) {
                ...
                // Set building
                $('#building_id').val(damage.building_id);
                
                // Set technician
                if (damage.technician_id) {
                    $('#technician_id').val(damage.technician_id);
                }
                
                // Set report date
                if (damage.report_date_jalali) {
                    $('#report_date').val(damage.report_date_jalali);
                }
                
                // Set visit date
                if (damage.visit_date_jalali) {
                    $('#visit_date').val(damage.visit_date_jalali);
                }
                
                // Set description
                $('#description').val(damage.description);
            } else {
                swal({
                    title: 'خطا',
                    text: 'خطا در بارگذاری گزارش خرابی',
                    type: 'error',
                    padding: '2em'
                }).then(function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                swal({
                    title: 'موفقیت',
                    text: response.message || 'گزارش خرابی با موفقیت به‌روزرسانی شد',
                    type: 'success',
                    padding: '2em',
                    timer: 2000
                }).then(function() {
                ...
                swal({
                    title: 'خطا',
                    text: response.message || 'خطا در به‌روزرسانی گزارش خرابی',
                    type: 'error',
                    padding: '2em'
                });
        ...
        error: function(xhr) {
            ...
            let errorMessage = 'خطا در به‌روزرسانی گزارش خرابی';
            if (response?.errors) {
